...
namespace VRisingNewMoon
{
    public unsafe static class NativeNetSharp
    {
        ...
        public static HookEngine hooks = new HookEngine();
        public static T Detour<T>(IntPtr from, T to) where T : Delegate
        {
            var hook = hooks.CreateHook(from, to);
            hooks.EnableHook(hook);
            return hook;
        }
